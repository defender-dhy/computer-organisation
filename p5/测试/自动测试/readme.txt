...
5. 	使用方法：
	1.确保cpu能实现多出来的两条指令addi和jalr，且将DM的寄存器扩大到0x2fff
	2.确保cpu的testbench命名为mips_tb.v，并进行一次仿真（或者可以对AutoTest里相关代码做修改，虽然我不会）
	3.将rar中所有文件解压到CPU的项目下（与.v文件同一级）
	4.打开AutoTest.cpp文件，修改isedir为你本地ISE的位置
	5.根据自己的情况修改filecompare函数
	6.编译运行等待ak